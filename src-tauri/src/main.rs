...
// Convert rdev Key to human-readable string
...
// Set up global input capture in a separate thread
fn setup_input_capture(window: Window) {
    thread::spawn(move || {
        let callback = move |event: Event| {
            ...
            match event.event_type {
                ...
                EventType::ButtonPress(button) => {
                    // Get current cursor position (rdev doesn't provide it with button events)
                    let button_str = button_to_string(button);
                    let event_data = MouseButtonEvent {
                        button: button_str,
                        x: 0.0, // Note: rdev doesn't provide position with button events
                        y: 0.0,
                        timestamp,
                    };
        ...
        // Start listening (this blocks the thread)
...
fn has_global_input() -> bool {
    // rdev is always available when compiled
    true
}

#[tauri::command]
fn is_readonly() -> bool {
    // For Tauri, we'll make this configurable later
    // For now, return false (interactive mode)
    false
}
...
fn main() {
    tauri::Builder::default()
        .setup(|app| {
            ...
            // Note: This may not work on all compositors (especially Wayland)
            #[cfg(target_os = "linux")]
            {
                use tauri::Manager;
                // Attempt to set click-through, but don't fail if it doesn't work
