...
window.addEventListener("load", function () {
    ...
    userEditModeInteractionsController.setOnShowPropertyEdit((obj) => {
        ...
        uiHelpers.propertyEditor.showPropertyEdit(objConfig, obj, objArrayIdx, configManager, () => {
...
function createDefaultConfig(canvas) {
    const objects = [
...
function createUIHelpers(canvas, configManager, userEditModeInteractionsController) {
    ...
    function showBothPanels() {
        ...
        propertyEditor.showCanvasConfig(configManager.config, canvas, (config) => {
    ...
    function createObject(type) {
        ...
        const schema = CANVAS_OBJECT_SCHEMAS[type];
        if (!schema) {
        ...
        const config = { [type]: innerConfig };
        configManager.addObject(config);
    }
    // Phase2: Delete object by index
    function deleteObjectAtIndex(objArrayIdx) {
        const config = configManager.config;
        if (objArrayIdx < 0 || objArrayIdx >= config.objects.length)
            return;
