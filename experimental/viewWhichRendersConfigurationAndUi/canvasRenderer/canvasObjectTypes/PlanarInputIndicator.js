...
import { PlanarInputIndicatorSchema } from '../../../modelToSaveCustomConfigurationLocally/configSchema';
export class PlanarInputIndicator extends CanvasObjectInstance {
    ...
    update() {
        ...
        if (keyboard) {
            ...
            if (xKeyPos && keyboard[xKeyPos])
                xAxis += 1;
            if (xKeyNeg && keyboard[xKeyNeg])
                xAxis -= 1;
            if (yKeyPos && keyboard[yKeyPos])
                yAxis += 1;
            if (yKeyNeg && keyboard[yKeyNeg])
                yAxis -= 1;
        }
        ...
        if (this.config.input.invertX)
            xAxis *= -1;
        if (this.config.input.invertY)
            yAxis *= -1;
...
//# sourceMappingURL=PlanarInputIndicator.js.map